...
class BambuDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass, *, entry: ConfigEntry) -> None:
        ...
        super().__init__(hass, LOGGERFORHA, name=DOMAIN)

        self.hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP, self._async_shutdown)
        self._service_call_listener = self.hass.bus.async_listen(
            SERVICE_CALL_EVENT, self._handle_service_call_event
        )
    ...
    def event_handler_internal(self, event: str):
        ...
        if event == "event_printer_bambu_authentication_failed":
            ...
            if self.latest_usage_hours != self.get_model().info.usage_hours:
                ...
                LOGGER.debug(
                    f"OVERWRITING USAGE_HOURS WITH : {self.latest_usage_hours}"
                )
    ...
    def _is_service_call_for_me(self, data: dict):
        ...
        if device_id is None and entity_id is None:
            LOGGER.error(
                f"Invalid data payload, neither device_id or entity_id provided: {data}"
            )

        if device_id is not None:
            if entity_id is not None:
                LOGGER.error(
                    "Either a device_id or an entity_id must be provided for a service call, not both."
                )
                return False
    ...
    async def _handle_service_call_event(self, event: Event) -> Any:
        ...
        if write_action:
            if self.get_model().print_fun.mqtt_signature_required:
                LOGGER.error(
                    "Printer firmware requires mqtt encryption. All control actions are blocked."
                )
    ...
    def _service_call_skip_objects(self, data: dict):
        ...
        try:
            ...
            LOGGER.error(
                f"Invalid objects value, should be comma separated ID list: {objects!r}"
            )
            return
    ...
    def _service_call_extrude_retract(self, data: dict) -> dict:
        ...
        if move not in ["EXTRUDE", "RETRACT"]:
            ...
            return {"Success": False, "Error": "Invalid type specified: '{move}'."}

        nozzle_temp = self.get_model().temperature.active_nozzle_temperature
        if force is not True and nozzle_temp < 170:
            LOGGER.error(
                f"Nozzle temperature too low to perform extrusion: {nozzle_temp}ºC"
            )
            return {
                "Success": False,
    ...
    def _get_ams_and_tray_index_from_entity_entry(self, ams_device, entity_entry):
        ...
        LOGGER.debug(
            f"FINAL TRAY VALUE: {full_tray + 1}/16 = Tray {tray + 1}/4 on AMS {ams_index}"
        )
    ...
    def _service_call_filament_drying(self, data: dict):
        ...
        if start_command:
            ...
            command["print"][
                "cooling_temp"
            ] = 45  # Must be at least 45 or the command is ignored
    ...
    def _service_call_read_rfid(self, data: dict):
        ...
        ams_index, tray_index = self._get_ams_and_tray_index_from_entity_entry(
            ams_device, entity_entry
        )
    ...
    def _service_call_load_filament(self, data: dict):
        ...
        if not self.get_model().supports_feature(Features.AMS_SWITCH_COMMAND):
            LOGGER.error(
                f"Loading filament is not available for this printer's firmware version, please update it"
            )
            return False
        ...
        if entity_unique_id.endswith("_external_spool"):
            ...
            for i, ext_spool in enumerate(self.get_model().external_spool):
                ...
                if vtray["identifiers"] == ams_device.identifiers:
                    ...
                    if data.get("temperature") is None and not ext_spool.empty:
                        temperature = (
                            int(ext_spool.nozzle_temp_min)
                            + int(ext_spool.nozzle_temp_max)
                        ) // 2
                    break
        elif not self.get_model().supports_feature(Features.AMS):
            LOGGER.error(f"AMS not available")
            ...
            ams_index, tray = self._get_ams_and_tray_index_from_entity_entry(
                ams_device, entity_entry
            )
            if ams_index is None:
                LOGGER.error("Unable to locate AMS.")
            ...
            if data.get("temperature") is None:
                temperature = (
                    int(ams_tray.nozzle_temp_min) + int(ams_tray.nozzle_temp_max)
                ) // 2
        else:
            LOGGER.error(f"An AMS tray or external spool is required")
            return False
    ...
    def _service_call_print_project_file(self, data: dict):
        ...
        if "//" in filepath:
            ...
            if self.config_entry.data["device_type"] in [
                Printers.H2S,
                Printers.H2D,
                Printers.P2S,
            ]:
    ...
    def _printer_ready(self):
        ...
        for device in dev_reg.devices.values():
            if config_entry_id in device.config_entries:
                ...
                if (
                    device.model == "AMS"
                    or device.model == "AMS Lite"
                    or device.model == "AMS 2 Pro"
                    or device.model == "AMS HT"
                ):
                    # And it's an AMS device
                    ams_serial = list(device.identifiers)[0][1]
                    if ams_serial not in existing_ams_devices:
                        LOGGER.debug(
                            f"Found stale attached AMS with serial {ams_serial}"
                        )
    ...
    def _report_generic_issue(self, issue: str, force: bool = False):
        ...
        issue_registry.async_create_issue(
            ...
            translation_placeholders={
                "device": f"'{self.config_entry.options.get('name', '')}'"
            },
        )
    ...
    def get_file_cache_directory(self, serial: str | None = None) -> str:
        ...
        try:
            ...
            LOGGER.debug(
                f"Default media directory not writable, falling back to local media directory : '{media_dir}'"
            )
    ...
    async def clear_file_cache(self, file_type: str = "all") -> Dict[str, Any]:
        ...
        if not cache_dir or not os.path.exists(cache_dir):
            return {
                "success": False,
                "error": "File cache not enabled or directory not found",
            }
